...
| 5 | Migration | Schema-versioned, pre-backup, abort-if-backup-fails, return-original-on-failure, three-level idempotent, defensive null/array checks; runs after existing per-load defaults; bumps `schemaVersion` last; unconditional chain synthesis on upgrade path; fresh installs get a generic English example chain via `getDefaultData()` |
| 6 | Reachability | Generic English `Example: 3-step follow-up` chain in `getDefaultData()`; legacy `Rückbucher (legacy)` chain synthesized only by upgrade migration; migration block carries a justifying comment explaining why German strings are preserved verbatim |

## Data shapes (concrete)

### Top-level workspace data after v2
...
```
1. If workspace-data.json missing → return getDefaultData() (already at schemaVersion 2)
2. Read file → JSON.parse. On error: log, return getDefaultData()
3. If raw.schemaVersion < 2 (or missing):
     → backupBeforeV2Migration()
     → if backup fails: log, return raw UNTOUCHED (skip migration entirely)
4. migrateData(raw):
     → existing per-load defaults (unchanged)
     → if schemaVersion < 2: migrateToV2(raw); raw.schemaVersion = 2
   On migration throw: log, return raw (possibly partially mutated; schemaVersion not bumped → retries next load)
5. If JSON.stringify(migrated) !== rawText: persist to disk
6. Return migrated
```

### `migrateToV2(data)` body

```js
function migrateToV2(data) {
  // ONE-SHOT HISTORICAL MIGRATION — strings preserved verbatim from the
  // pre-v0.1 private German "Rückbucher" workflow so existing user todos
  // stay display-consistent with the synthesized chain template. Public
  // feature code (Settings UI, spawn UX, runtime rendering) contains no
  // such strings; this block is the only place "Rückbucher" appears in
  // post-cleanup source. Runs once per workspace; idempotent at three
  // independent levels (schemaVersion gate, chain dedup, todo kind check).
  try {
    if (!Array.isArray(data.escalationChains)) data.escalationChains = [];

    if (!data.escalationChains.find(c => c.id === 'chain-rueckbucher-legacy')) {
    ...
    console.error('migrateToV2 failed; data left as-is for next load:', e);
    throw e;  // bubble so loadData catches and returns unmigrated raw
  }
}
```

### `backupBeforeV2Migration()` body

```js
function backupBeforeV2Migration() {
  const backupsDir = path.join(app.getPath('userData'), 'backups');
  try { fs.mkdirSync(backupsDir, { recursive: true }); } catch {}
  const ts = new Date().toISOString().replace(/[:.]/g, '-');  // 2026-05-08T14-23-45-123Z
  const backupPath = path.join(backupsDir, `workspace-data.pre-migration-v2.${ts}.json`);
...
### Three-level idempotence guarantee

Even if `data.schemaVersion` is corrupted/missing/manually deleted between loads:

1. **Schema gate** — primary; `schemaVersion >= 2` skips migration entirely
2. **Chain dedup** — `find(c => c.id === 'chain-rueckbucher-legacy')` prevents duplicate insert
...
|---|---|---|
| [main.js](main.js) | `migrateToV2`, `backupBeforeV2Migration`, hardened `loadData`, `schemaVersion` in `getDefaultData`, generic example chain in `getDefaultData` | **+90 / -5** |
| [preload.js](preload.js) | None | 0 |
| [package.json](package.json) | None | 0 |
| [src/01-types-state.js](src/01-types-state.js) | JSDoc typedefs `EscalationChain`, `EscalationChainItem`, `Offset`; add `schemaVersion`, `escalationChains` to `WorkspaceData` typedef | **+30** |
...
| [src/11-projects-init.js](src/11-projects-init.js) | Remove `isRueckbucherButtonEnabled`, `setRueckbucherButtonEnabled`, `isRueckbucherBoxCollapsed`, `setRueckbucherBoxCollapsed`, `spawnRueckbucherFollowups`, `addWorkdays` STAYS (used by `applyOffset`); add `applyOffset`, `spawnChain`, `escalationBoxCollapsedHelpers` | **+70 / -55** |
| [app.js](app.js) | Remove ~150 lines of Rückbucher-specific render block (lines 5320-5470) including `showRueckbucherBtn` gate (no longer needs `window.api?.isPersonalBuild` reference); add `spawn chain` dropdown button + dropdown menu render; add per-chain collapsible boxes for future-dated items; add palette `spawn chain` command | **+170 / -150** |
...
| [tests/pure.test.js](tests/pure.test.js) | Tests for `applyOffset` (8-10 cases), `migrateToV2` (5-7 cases) | **+150** |

**Net:** roughly **+755 / -237 = +518 LOC** across 6 files. Two-thirds is feature implementation; one-third is migration + tests.

## Implementation phases

### Phase A — Data foundation (45-90 min)
1. Add `schemaVersion: 2` to `getDefaultData()` in main.js
2. Add `escalationChains: [<example chain>]` to `getDefaultData()`
3. Add `migrateToV2` function in main.js (after `migrateData`)
4. Wire schema-version gate into `migrateData`'s body (after existing per-load defaults)
5. Add `backupBeforeV2Migration` function in main.js
6. Harden `loadData` with new error handling: try/catch around parse, backup, migrate; return `raw` on backup failure or migration throw
7. Add JSDoc types to src/01-types-state.js for `EscalationChain`, `EscalationChainItem`, `Offset`; extend `WorkspaceData`
8. **Smoke test** — `npm start`, then verify the four points below in order. **Do not greenlight Phase B until all four pass.** A backup that merely exists but isn't restorable defeats the purpose.
   ...
   **Backup verification (mandatory):**
   ...
   a. **Path exists.** PowerShell:
      ```powershell
      Get-ChildItem "$env:APPDATA\workspacehub\backups\workspace-data.pre-migration-v2.*.json" | Select-Object Name, Length, LastWriteTime
      ```
   ...
   b. **File is readable AND parses as valid JSON.** PowerShell:
      ```powershell
      $bk = Get-ChildItem "$env:APPDATA\workspacehub\backups\workspace-data.pre-migration-v2.*.json" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
   ...
   **Migration verification:**
   ...
   - DevTools console: `state.data.schemaVersion` returns `2`; `state.data.escalationChains` is an array; the array contains an object with `id === 'chain-rueckbucher-legacy'` and the three German item titles.
...
2. Add tests in tests/pure.test.js:
   ...
   - `migrateToV2` idempotence: running twice produces same data
   - `migrateToV2` synthesizes chain even when no rueckbucher todos exist
   - `migrateToV2` re-tags todos correctly
   - `migrateToV2` defensive: malformed `proj.todos` doesn't throw
...
2. Migration replay: restore your `workspace-data.pre-migration-v2.<ts>.json` backup, relaunch app, verify migration runs and produces identical result (idempotence stress test)
...
# Expected: only main.js's migrateToV2 chain template
```
...
3. Verify migration backup exists at `%APPDATA%/workspacehub/backups/workspace-data.pre-migration-v2.<ts>.json`
...
- User's data: pre-migration backup exists at `%APPDATA%/workspacehub/backups/workspace-data.pre-migration-v2.<ts>.json`. To restore: copy back over `workspace-data.json`, relaunch app pre-revert. After revert, the legacy Rückbucher code is gone but the data still has the v1 shape — perfectly readable (the `kind:'rueckbucher'` rendering code is gone but todos render as plain todos via the default path).
...
**Approved by user on 2026-05-08** with two amendments folded into the document:
